	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (loginuser != null && "admin".equals(loginuser.getUserid())) {
			...
			if (!"POST".equalsIgnoreCase(method)) {
				...
				ArrayList<SpecVO> specList = pdao.selectSpecList();
				request.setAttribute("specList", specList);
				super.setRedirect(false);
				super.setViewPage("/WEB-INF/myshop/admin/productRegister.jsp");

			} else {
				// post 방식일 경우(form에 입력받은 데이터를 전송해야 함)
				/*
				 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 파일을 첨부해서 보내는 폼태그가
				 * enctype="multipart/form-data" 으로 되어었다라면 HttpServletRequest
				 * request 을 사용해서는 데이터값을 받아올 수 없다. 이때는 cos.jar 라이브러리를 다운받아 사용하도록
				 * 한 후 아래의 객체를 사용해서 데이터 값 및 첨부되어진 파일까지 받아올 수 있다.
				 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				 */
			...
			HttpSession sesssion = request.getSession();
				 ...
				 * saveDirectory, -- 파일이 저장될 경로 int maxPostSize, -- 업로드할 파일 1개의
				 * 최대 크기(byte) String encoding, FileRenamePolicy policy) -- 중복된
				 * 파일명이 올라갈 경우 파일명다음에 자동으로 숫자가 붙어서 올라간다.
				 * 
				 * 파일을 저장할 디렉토리를 지정할 수 있으며, 업로드제한 용량을 설정할 수 있다.(바이트단위). 이때 업로드
				 * 제한 용량을 넘어서 업로드를 시도하면 IOException 발생된다. 또한 국제화 지원을 위한 인코딩 방식을
				 * 지정할 수 있으며, 중복 파일 처리 인터페이스를사용할 수 있다.
				 * 
				 * 이때 업로드 파일 크기의 최대크기를 초과하는 경우이라면 IOException 이 발생된다. 그러므로
				...
				// === 첨부 이미지 파일을 올렸으니 그 다음으로 제품정보를 (제품명, 정가, 제품수량,...) DB의 tbl_product 테이블에 insert 를 해주어야 한다.  ===
					// 새로운 제품 등록 시, form 태그에서 입력한 값들을 얻어오기
					...
					*/
					  ...
					  // 크로스 사이트 스크립트 공격에 대응하는 안전한 코드(시큐어 코드)로 작성하기
					  ...
					  int m = 1;
					  //String[] arr_attach = mtrequest.getParameterValues("attach");
					  String str_attachCount = mtrequest.getParameter("attachCount");	// 추가 이미지 갯수는 0~10까지 이다.
					  int attachCount = Integer.parseInt(str_attachCount);
					  
					  for(int i=0; i<attachCount; i++){
					  //for(int i=0; i<arr_attach.length; i++){
						  String attachFileName = mtrequest.getFilesystemName("attach"+i);
						  //String attachFileName = mtrequest.getFilesystemName(arr_attach[i]);
