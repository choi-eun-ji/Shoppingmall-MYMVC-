...
@WebServlet(
      ...
      urlPatterns = { "*.up" }, 
      initParams = { 
            @WebInitParam(name = "propertyConfig", value = "C:/NCS/workspace_jsp/MyMVC/WebContent/WEB-INF/Command.properties", description = "*.up 에 대한 클래스의 매핑파일")
      })
public class FrontController extends HttpServlet {
   ...
   public void init(ServletConfig config) throws ServletException {
	  ...
	  /*
               ...
               그러므로 init 메소드에는 frontController 서블릿이 동작해야할 환경설정을 잡아주는데 사용된다.  
      */
      // *** 확인용 *** //
      ...
      // 특정 파일에 내용을 읽어 오기 위한 용도로 쓰인 객체 
      try{
         String props =config.getInitParameter("propertyConfig");
         // System.out.println(pops);
         fis = new FileInputStream(props);   // fis 는 "C:/NCS/workspace_jsp/MyMVC/WebConte/Command.properties" 파일의 내용을 읽어오기 위한 용도로 쓰이는 객체이다. 
         pr.load(fis); // fis 객체를 사용하여 "C:/NCS/workspace_jsp/MyMVC/WebConte/Command.properties" 파일의 내용을 읽어다가 properties 클래스의 객체인 pr에 로드시킨다. 
                    // 그러면 pr은 읽어온 파일 command.properties의 내용에서 =를 기준으로 왼쪽은 key로 인식하고 value로 인식한다.
         Enumeration<Object> en = pr.keys();
         /*
          * pr.keys(); 은 "C:/NCS/workspace_jsp/MyMVC/WebConte/Command.properties" 파일의 내용물에서 =를 기준으로 왼쪽에 있는 모든 key들만 가져오는 것이다.
          */
         ...
         while(en.hasMoreElements()){
            ...
            System.out.println("~~~~ 확인용 value => " + pr.getProperty(key));   // getProperty : properties의 value를 가져오게 하는 메소드이다.
            [실행결과]
            ~~~~ 확인용 key => /main.up
            ...
            */
            ...
            if(className != null){
            	...
            	// 클래스로부터 실제 객체(인스턴스)를 생성해주는 것이다. -> 객체를 생성해야지 클래스를 호출할 수 있다. 
            	...
            	*/
            	...
            	// cmdMap 에서 키값으로 Command.properties 파일에 저장되어진 url을 주면 cmdMap 에서 해당 클래스에 대한 객체(인스턴스)를 얻어오도록 만든 것이다.
            	cmdMap.put(key, obj);
            }
         }// end of while
         ...
         System.out.println(">>> C:/NCS/workspace_jsp/MyMVC/WebConte/Command.properties 파일이 없습니다.");
         e.printStackTrace();
      }catch(IOException e){
         e.printStackTrace();
      }catch(Exception e){
   ...
   private void requestProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    AbstractController action = (AbstractController) cmdMap.get(key);	// 다형성 index, maincontroller 이지만, 공통적으로 가져와야 하기때문에 이들의 부모 클래스인 abstractcontroller로 가져온다.
