show user;
-- USER이(가) "SYS"입니다.
drop user skyuniversity cascade;
create user logitech identified by cclass; 
grant connect, resource, create view, unlimited tablespace to logitech;

create user skyuniversity identified by cclass default tablespace users; 
grant connect, resource, create view, unlimited tablespace to skyuniversity;
...
,status             number(1) default 1 not null        -- 회원 탈퇴 유무 1: 사용가능(가입중) 0:사용불능(탈퇴)
,idle               number(1) default 0 not null        -- 휴면유무 0: 활동중 1: 휴먼중
...
select userid, name, email, mobile, postcode, address, detailaddress, extraaddress, gender, substr(birthday,1,4) as birthyyyy, substr(birthday,6,2) as birthmm, substr(birthday,9) as birthdd
,coin, point, to_char(registerday, 'yyyy-mm-dd') as registerday
,trunc(months_between(sysdate, lastpwdchangedate)) as pwdchangegap
from tbl_member
where status = 1 and userid = 'eomjh' and pwd = '4bc1edda2ffdd1b1e424e6f635e646dd39f758340c413c0f23f93fcd46a563b6'
...
-- 로그인시 "비밀번호를 변경하신지 3개월이 지났습니다. 암호를 변갱하세요 라는 alert를 띄우기 위해 아래와 같이 한다."
update tbl_member set registerday = add_months(registerday, -4)
                     , lastpwdchangedate = add_months(lastpwdchangedate, -4) 
where userid='eomjh';

-- 마지막으로 로그인 한지 12개월이 초과한 경우 또는 회원을 가입하고서 로그인을 하지 않은지가 12개월이 초과한 경우
-- 로그인시 "로그인을 한지 1년이 지나서 휴먼상태로 되었습니다. 관리자에게 문의 바랍니다"라는 alert를 띄우기 위해 아래와 같이 한다.
...
update tbl_member set registerday = add_months(registerday, -2)
                     , lastpwdchangedate = add_months(lastpwdchangedate, -2) 
where userid = 'youks';


select max(logindate)
from tbl_loginhistory
where fk_userid = 'hongkd'
...
select userid, name, email, mobile, postcode, address, detailaddress, extraaddress, gender, birthyyyy, birthmm, birthdd, coin, point, registerday, pwdchangegap, lastlogingap
from 
(
    select userid, name, email, mobile, postcode, address, detailaddress, extraaddress, gender
            ...
            , trunc(months_between(sysdate, lastpwdchangedate)) as pwdchangegap
    from tbl_member
    where status = 1 and userid = 'hongkd' and pwd = '9695b88a59a1610320897fa84cb7e144cc51f2984520efb77111d94b402a8382'
) M
cross join
(
    select trunc(months_between(sysdate, max(logindate))) as lastlogingap
...
"     , trunc( months_between(sysdate, lastpwdchangedate) ) AS pwdchangegap\n"+
"from tbl_member\n"+
"where status = 1 and  userid = 'hongkd' and pwd = '808031717126f4b3f774b0a734ac4ec75daddbf6c99a4a0fa20afbf152412f52'\n"+
") M \n"+
"CROSS JOIN \n"+
"(\n"+
"select trunc( months_between(sysdate, max(logindate)) ) AS lastlogingap\n"+
...
-- 오라클에서 프로시저를 사용하여 회원을 대량으로 입력(insert)하겠다.
...
begin
    for i in 1..100 loop
        insert into tbl_member(userid, pwd, name, email, mobile, postcode, address, detailaddress, extraaddress, gender, birthday)
        values(p_userid||i, '18006e2ca1c2129392c66d87334bd2452c572058d406b4e85f43c1f72def10f5', p_name||i, i||'7H1N6U9AXXKXfgzNclV1MLt7IatBXAecPq0RYJgVk5Q='||i, '/1i0EU411cNfObPxEFoOpg==', '14038', '경기 안양시 만안구 안양천서로 177', '211-222', ' (안양동, 래미안 안양 메가트리아)', p_gender, '1997-01-27');
    end loop;
...
begin
     for i in 1..100 loop
         insert into tbl_member(userid, pwd, name, email, mobile, postcode, address, detailaddress, extraaddress, gender, birthday) 
         values(p_userid||i, '18006e2ca1c2129392c66d87334bd2452c572058d406b4e85f43c1f72def10f5', p_name||i, '7H1N6U9AXXKXfgzNclV1MLt7IatBXAecPq0RYJgVk5Q==' , '/1i0EU411cNfObPxEFoOpg==', '15864', '경기 군포시 오금로 15-17', '102동 9004호', ' (금정동)', p_gender, '1991-01-27');
     end loop;
end pcd_member_insert;
...
where T.rno between 1 and 3; -- 1페이지 (한페이지당 3개를 보여줄때)

/*
    currenShowPageNo ==> 1
    sizePerPage  -- 3
    
   ( currenShowPageNo * sizePerPage ) - (sizePerPage -1) ==> 1
   ( currenShowPageNo * sizePerPage ) ==> 3
*/
...
/*
   ...
   ( currenShowPageNo * sizePerPage ) - (sizePerPage -1) ==> 4
   ( currenShowPageNo * sizePerPage ) ==> 6
*/
...
where T.rno between 7 and 9;    -- 3페이지 (한페이지당 3개를 보여줄때)

/*
    currenShowPageNo ==> 3
    sizePerPage  -- 3
    
   ( currenShowPageNo * sizePerPage ) - (sizePerPage -1) ==> 11
   ( currenShowPageNo * sizePerPage ) ==> 9
*/
...
alter table TBL_PRODUCT modify (SALEPRICE NUMBER(35));
COMMIT;
...
create table tbl_pro_dislike
(fk_userid      varchar2(20) not null
,fk_pnum        number(8) not null
,constraint PK_tbl_pro_dislike primary key(fk_userid, fk_pnum)
,constraint FK_tbl_pro_dislike_userid foreign key (fk_userid) references tbl_member(userid)
,constraint FK_tbl_pro_dislike_pnum foreign key (fk_pnum) references tbl_product(pnum)
);

select *
from dislike;

insert into tbl_product_like(fk_userid, fk_pnum) values('ejejc', 58);
...
select
    (select count(*)
    from tbl_product_like
    where fk_pnum = 58) as likecnt
    , ( select count(*)
        from tbl_product_dislike
        where fk_pnum = 58) as dislikecnt
from dual;
...
where fk_userid = 'ejejc' and fk_pnum='58'
insert into tbl_product_like(fk_userid, fk_pnum) values('ejejc',58);
...
String sql = "insert into tbl_product_like(fk_userid, fk_pnum) values('ejejc',58)";
rollback
