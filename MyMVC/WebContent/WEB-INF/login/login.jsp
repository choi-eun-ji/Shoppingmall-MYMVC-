...
<script type="text/javascript">
   ...
   $(document).ready(function() {
      ...
      // === 로컬스토리지(localStorage)에 저장된 키가 "saveid"인 userid 값을 불러와서 input 태그 userid에 넣어주기 === //
      var loginUserid = localStorage.getItem("saveid");
      
      // 로컬스토리지에 저장된 아이디 값이 있을 때
      if (loginUserid != null) {
         $("input#loginUserid").val(loginUserid);
         $("input:checkbox[id=saveid]").prop("checked", true);
      }else{   // 로컬스토리지에 저장된 아이디 값이 없을 때
   ...
   function goLogin() {
      ...
      if(loginUserid == ""){
         ...
         >> 로컬 스토리지(localStorage)와 세션 스토리지(sessionStorage) << 
            ...
            로컬 스토리지 => 영구저장(개발자가 삭제하기 전까지), 사용자 PC에 저장
            세션 스토리지 => 임시저장(웹브라우저를 닫을 시 사라짐), 사용자 PC에 저장
            세션 => 임시저장(웹브라우저를 닫을 시 사라짐), WAS PC 메모리에 저장
   ...
   }// end of fuction goCoinPurchaseTypeChoice(userid)--------------------------------------
	
   // 아임포트 결제를 해주는 함수
   function goCoinPurchaseEnd(userid, coinmoney) {
		alert("부모창의 함수 호출. 사용자ID: " + userid + "결제 금액: " + coinmoney);	
		
		// 아임포트 결제 팝업창 띄우기
		var url = "/MyMVC/member/coinPurchaseEnd.up?userid="+userid+"&coinmoney="+coinmoney;
   ...
   // DB상의 tbl_member 테이블에 해당 사용자의 코인금액을 증가시켜주는 함수
...
<c:if test="${ sessionScope.loginuser != null }">
 ...
 <%-- ****** 아이디 찾기 Modal ****** --%>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body" style="height: 300px; width: 100%;">
          <div id="idFind">
             <iframe style="border: none; width: 100%; height: 280px;" src="<%= request.getContextPath()%>/login/idFind.up"> <!-- GET 방식이다. -->
             </iframe>
          </div>
        </div>
  ...
  <!-- PG에 코인금액을 카드로 결제 후, DB상의 사용자의 코인액을 UPDATE 해주는 FORM -->
